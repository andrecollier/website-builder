...
async function compareFeatures2() {
  const browser = await chromium.launch();
  const context = await browser.newContext({
    viewport: { width: 1440, height: 900 }
  });

  // Reference - scroll to features section (more)
  ...
  // Scroll to features cards
  await refPage.evaluate(() => window.scrollTo(0, 1600));
  await refPage.waitForTimeout(500);
  await refPage.screenshot({ path: 'screenshots/ref-features2.png' });
  ...
  // Scroll to features
  await genPage.evaluate(() => window.scrollTo(0, 1600));
  await genPage.waitForTimeout(500);
  await genPage.screenshot({ path: 'screenshots/gen-features2.png' });

  await browser.close();
  console.log('Features 2 screenshots saved');
}

compareFeatures2().catch(console.error);
