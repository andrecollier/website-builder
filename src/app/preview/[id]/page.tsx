          ...
          <div className="flex items-center gap-4">
            ...
            {scaffoldSuccess && (
              <button
                type="button"
                onClick={() => {
                  const cmd = `cd Websites/${websiteId}/generated && npm run dev`;
                  navigator.clipboard.writeText(cmd);
                  alert(`Command copied to clipboard:\n\n${cmd}\n\nRun this in your terminal to preview the site.`);
                }}
                className={cn(
                  'px-3 py-1.5 text-sm font-medium rounded-lg',
                  'bg-[rgb(var(--muted))] text-[rgb(var(--foreground))]',
                  'hover:bg-[rgb(var(--muted)/0.8)]',
                  'focus:outline-none focus:ring-2 focus:ring-[rgb(var(--muted-foreground))]',
                  'flex items-center gap-1.5'
                )}
                title="Copy command to start preview server"
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
